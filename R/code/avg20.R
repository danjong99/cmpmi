...
set.seed(124)
setwd("~/Desktop/10x_scRNAseq/GF1_WT2_MergedRCC/SCENIC_R/Avg20_All/No4/")
avg20_4 <- read.csv("./source/avg20.4.csv")

rownames(avg20_4) <- avg20_4$GENE
avg20_4$GENE <- NULL
avg20_4 <- as.matrix(avg20_4)
avg20_4 <- avg20_4*20
head(avg20_4)

Exr <- readRDS("~/Desktop/10XResults/GF1_WT2_MergedRCC/GF1WT2/WT_GF_Combined_Seurat.rds")
#cellLabels <- Exr@meta.data[Exr@meta.data$res.1.2=="1"|Exr@meta.data$res.1.2=="2"|Exr@meta.data$res.1.2=="3"|Exr@meta.data$res.1.2=="4"|Exr@meta.data$res.1.2=="6"|Exr@meta.data$res.1.2=="7"|Exr@meta.data$res.1.2=="11",]
cellLabels <- Exr@meta.data
rm(Exr)
...
index4 <- intersect(rownames(cellLabels),colnames(avg20_4))
cellLabels4 <- cellLabels[index4,]
cellLabels4$cellID <- NULL
avg20_4 <- avg20_4[,index4]
length(colnames(avg20_4))

avg20_sceMP4 <- SingleCellExperiment(assays = list(counts = avg20_4),
                                     colData=data.frame(cellLabels4[colnames(avg20_4),, drop=FALSE]))
dir.create("data")
save(avg20_sceMP4, file="./data/avg20_sceMP4.RData")
exprMat <- counts(avg20_sceMP4)
dim(exprMat)

cellInfo <- colData(avg20_sceMP4)
cellInfo$nGene <- colSums(exprMat>0)
cellInfo <- data.frame(cellInfo)
head(cellInfo)
setwd("/Volumes/Extreme SSD/scRNAseq_analysisfile/10XResults/SPFGFPhago/SCENIC_R/Avg20_All/No4")
saveRDS(cellInfo, file="./int/cellInfo.Rds")
org="mgi" # or hgnc, or dmel
dbDir="/Volumes/Extreme SSD/scRNAseq_analysisfile/10XResults/SPFGFPhago/SCENIC_R/All/databases" # RcisTarget databases location
myDatasetTitle="SCENIC Colon Phagocytes" # choose a name for your analysis
